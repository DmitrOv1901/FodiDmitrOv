...
float3 ApplyTerrainDecal(float3 baseColor, float2 localUV, float packedPlacement)
{
    ...
    uint variant = code & 3u;
    uint rotation = (code >> 2u) & 3u;
    bool mirror = ((code >> 4u) & 1u) != 0u;
    float2 transformedUV = TerrainTransformDecalUV(localUV, rotation, mirror);

    // Four horizontal 32x32 slots in a 128x32 atlas. Sampling texel centres
    ...
    float2 atlasUV = pixel / float2(128.0, 32.0);
