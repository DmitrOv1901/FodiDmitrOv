...
- Локальные проверки теперь запускаются из `tools/lighting-tests/Fodinae.LightingTests.csproj`: layout/binding, 5 CPU golden-сцен, streaming governor, native HLSL transport (3117 проверок) и shader equivalence. Отдельный terrain-transition harness в этот перенос не включён; production terrain tests остаются в `tools/Fodinae.TerrainTests`.
