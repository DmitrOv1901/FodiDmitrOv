...
- [x] Разбить `Kern.Runtime` на Core/Application/Infrastructure/Presentation.
...
- [ ] Переполнение текста проверяется вручную. Детектор макета — кнопка в
      дев-панели, замер геометрии (`docs/design/design-text-overflow.md`) — прогон в
...
- [ ] `visual/main-menu-mirror/tools/inventory-art.py`: карта всех 326 PNG
      ...
      `docs/design/design-art-inventory.md` машинным файлом. Сейчас единственный способ
